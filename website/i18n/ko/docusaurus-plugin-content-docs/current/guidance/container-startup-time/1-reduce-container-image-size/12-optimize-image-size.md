...
다양한 AI/ML 프레임워크와 플랫폼은 편의성을 제공하고 실험을 가능하게 하는 즉시 사용 가능한 컨테이너 이미지를 제공합니다. 그러나 이러한 이미지는 가능한 한 넓은 기능 세트를 다루려고 하므로 다양한 런타임, 프레임워크 또는 지원되는 API를 포함할 수 있어 프로덕션 컨테이너 이미지에 적합하지 않은 비대화로 이어질 수 있습니다.
...
프로덕션 지향적인 포괄적인 접근 방식은 다음을 포함합니다:
