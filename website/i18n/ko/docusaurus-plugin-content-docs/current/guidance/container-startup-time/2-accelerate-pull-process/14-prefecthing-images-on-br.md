---
sidebar_label: 데이터 볼륨에 컨테이너 이미지 미리 로드
---

# EBS 스냅샷으로 Bottlerocket 데이터 볼륨에 컨테이너 이미지 미리 로드

이 패턴의 목적은 Bottlerocket OS의 데이터 볼륨에 이미지를 캐싱하여 대용량 이미지를 가진 컨테이너의 콜드 스타트 시간을 줄이는 것입니다.

데이터 분석 및 머신 러닝 워크로드는 종종 대용량 컨테이너 이미지(보통 기가바이트 단위)를 필요로 하며, Amazon ECR이나 다른 이미지 레지스트리에서 풀하고 추출하는 데 수 분이 걸릴 수 있습니다. 이미지 풀 시간을 줄이는 것이 이러한 컨테이너 시작 속도를 개선하는 핵심입니다.

Bottlerocket OS는 AWS가 특별히 컨테이너 실행을 위해 구축한 Linux 기반 오픈 소스 운영 체제입니다. OS 볼륨과 데이터 볼륨의 두 볼륨을 가지며, 후자는 아티팩트와 컨테이너 이미지를 저장하는 데 사용됩니다. 이 샘플은 이미지를 풀하고 나중에 사용하기 위해 스냅샷을 찍기 위해 데이터 볼륨을 활용합니다.

EBS 스냅샷에 이미지를 캐싱하고 EKS 클러스터에서 시작하는 프로세스를 보여주기 위해 이 샘플은 Amazon EKS 최적화 Bottlerocket AMI를 사용합니다.

자세한 내용은 GitHub 샘플 및 블로그 게시물을 참조하십시오:
- [GitHub - AWS Bottlerocket 인스턴스를 위한 컨테이너 이미지 캐싱](https://github.com/aws-samples/bottlerocket-images-cache/tree/main)
- [블로그 게시물 - Bottlerocket 데이터 볼륨으로 Amazon EKS에서 컨테이너 시작 시간 줄이기](https://aws.amazon.com/blogs/containers/reduce-container-startup-time-on-amazon-eks-with-bottlerocket-data-volume/)

## 이 스크립트 개요

![](img/bottlerocket-image-cache.png)

1. EKS용 Bottlerocket AMI로 EC2 인스턴스를 시작합니다.
2. Amazon System Manager를 통해 인스턴스에 접근합니다.
3. Amazon System Manager Run Command를 사용하여 이 EC2에서 캐시할 이미지를 풀합니다.
...
# Amazon EKS 및 Karpenter와 함께 스냅샷 사용

Karpenter 노드 클래스에서 `snapshotID`를 지정할 수 있습니다(또 다른 옵션은 이 저장소의 Infrastructure Blueprints를 사용하고 IaC 구성의 일부로 [snapshotID](https://github.com/awslabs/ai-on-eks/blob/main/infra/base/terraform/variables.tf#L64-L68)를 전달하는 것입니다). EC2NodeClass에 콘텐츠를 추가합니다:
